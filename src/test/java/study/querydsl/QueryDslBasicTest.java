...
public class QueryDslBasicTest {
    ...
    //필드 레벨로 가져가도, 동시성 문제 같은거 고민안하고 사용하면 된다.
    //스프링 프레임워크가 주입해주는 엔티티 매니저 자체가 멀티 쓰레드에 문제 없게 설계가 되어있다.
    ...
    void search() {
        Member findMember = queryFactory
                .selectFrom(member) //select, from 을 합쳐서 사용 가능
    ...
    void searchAndParam() {
        Member findMember = queryFactory
                ...
                .where(member.username.eq("member1"),
                        //where절 조건이 and 로 이어지는 경우 , 를 사용해서 조건을 구분할 수도 있다.
                        member.age.eq(10))
                .fetchOne();
     ...
     * 1. 회원 나이 내림차순(desc)
     * 2. 회원 이름 오름차순(asc)
     * 단, 2에서 이름이 null 이면 마지막에 출력(null last)
     */
    @Test
    void sort() {
